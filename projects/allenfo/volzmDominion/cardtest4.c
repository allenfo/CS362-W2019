...
#define NOISY_TEST 1
	...
	test1=cardEffect(mine,smithy,gold,silver,&testG,handPos,0);
	...
	test2=cardEffect(mine,gold,28,silver,&testG,handPos,0);
	printf("expected return -1, actual was %d\n",test2);
	testassert(-1,test2);
#endif
#if NOISY_TEST==1
	...
	testG.hand[currentPlayer][0]=gold;
	test3=cardEffect(mine,gold,copper,silver,&testG,handPos,0);
	printf("expected return -1, actual was %d\n",test3);
	testassert(-1,test3);
#endif
#if NOISY_TEST==1
	...
	test4=cardEffect(mine,copper,village,silver,&testG,handPos,0);
	printf("expected return -1, actual was %d\n",test4);
	testassert(-1,test4);
#endif
#if NOISY_TEST==1
	...
	testG.hand[currentPlayer][0]=copper;
	cardEffect(mine,copper,village,silver,&testG,handPos,0);
